  ...
  var mobile = (function (domGlobals) {
    ...
    var focusIn$1 = function (component, flowConfig) {
      flowConfig
        ...
        .or(descendant$1(component.element(), flowConfig.selector()))
        .each(function (first) {
    ...
    var doMove$2 = function (ifCycle, ifMove) {
      return function (element, focused, matrixConfig) {
        ...
        return closest$1(focused, matrixConfig.selectors().row()).bind(
          function (inRow) {
            var cellsInRow = descendants(
              inRow,
              matrixConfig.selectors().cell()
            );
            return findIndex$1(cellsInRow, focused).bind(function (colIndex) {
              var allRows = descendants(
                element,
    ...
    var combine = function (detail, data, partSpec, partValidated) {
      ...
      return deepMerge(
        ...
        { "debug.sketcher": wrap$1("part-" + data.name(), spec) }
      );
    };
    ...
    var single$1 = function (owner, schema, factory, spec) {
      ...
      return deepMerge(factory(detail, specWithUid), {
        "debug.sketcher": wrap$1(owner, spec),
      });
    };
    ...
    var getValue$1 = function (component, repConfig, repState) {
      ...
      return readOptFrom$1(dataset, key).fold(
        function () {
    ...
    var sketch = function (detail, components, spec, externals) {
      ...
      var getXOffset = function (slider, spectrumBounds, detail) {
        ...
        if (v < detail.min()) {
          ...
          return (
            ((detail.value().get() - detail.min()) / range) *
    ...
    var getCurrentRoute = function (comp, transConfig, transState) {
      var el = comp.element();
      return has(el, transConfig.destinationAttr())
        ? Option.some({
            start: constant(get(comp.element(), transConfig.stateAttr())),
    ...
    var jumpTo = function (comp, transConfig, transState, destination) {
      ...
      if (
        has(comp.element(), transConfig.stateAttr()) &&
    ...
    var getState = function (comp, transConfig, transState) {
      var e = comp.element();
      return has(e, transConfig.stateAttr())
      ...
      var memMenu = record(
        tieredMenu.sketch({
          ...
          onOpenSubmenu: function (container, item, submenu) {
            var w = get$6(container.element());
            var menu = ancestor$1(item.element(), '[role="menu"]').getOrDie(
              "hacky"
            );
          ...
          onCollapseMenu: function (container, item, menu) {
            var submenu = ancestor$1(item.element(), '[role="menu"]').getOrDie(
              "hacky"
            );
