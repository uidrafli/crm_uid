  ...
  var template = (function () {
    ...
    var insertTemplate = function (editor, ui, html) {
      ...
      html = replaceTemplateValues(
        editor,
        html,
        Settings.getTemplateReplaceValues(editor)
      );
      ...
      global$1.each(dom.select("*", el), function (n) {
        if (
          hasClass(
            n,
            Settings.getCreationDateClasses(editor).replace(/\s+/g, "|")
          )
        ) {
    ...
    var register = function (editor) {
      editor.addCommand(
        "mceInsertTemplate",
        curry(Templates.insertTemplate, editor)
      );
    };
    ...
    var insertIframeHtml = function (editor, win, html) {
      ...
      html = Templates.replaceTemplateValues(
        editor,
        html,
    ...
    var register$1 = function (editor) {
      editor.addButton("template", {
        title: "Insert template",
        onclick: Templates.createTemplateList(
          editor.settings,
          showDialog(editor)
        ),
      });
