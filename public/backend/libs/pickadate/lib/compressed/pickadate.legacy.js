    ...
    Picker = function ($ELEMENT, SETTINGS) {
      var // Pseudo picker constructor
        ...
        P = (Picker.prototype = {
            ...
            $ELEMENT
              .on({
                ...
                change: function () {
                  ...
                  if (ELEMENT_HIDDEN) {
                    ELEMENT_HIDDEN.value = ELEMENT.value
                      ? getDateFormatted(SETTINGS.formatSubmit)
                      : "";
                  }
                },
                keydown: function (event) {
                  var // Grab the keycode
                  ...
                  if (
                    isKeycodeDelete ||
                    (!CALENDAR.isOpen && KEYCODE_TO_DATE[keycode])
                  ) {
          ...
          open: function () {
            ...
            $document
              .on("focusin.P" + CALENDAR.id, function (event) {
                ...
                if (
                  !$HOLDER.find(event.target).length &&
                    ...
                    setDateSelected(
                      createValidatedDate(
                        [
                          MONTH_FOCUSED.YEAR,
                          MONTH_FOCUSED.MONTH,
                          DATE_HIGHLIGHTED.DATE + keycodeToDate,
                        ],
                        keycodeToDate
                      ),
                      1
          ...
          setDate: function (year, month, date, isSuperficial) {
            setDateSelected(
              createValidatedDate([year, --month, date]),
              isSuperficial
            );
            return P;
          }, //setDate
          ...
          return {
            ...
            ddd: function (string) {
              ...
              return string
                ...
                : SETTINGS.weekdaysShort[this.DAY];
            },
            dddd: function (string) {
              ...
              return string
                ? getFirstWordLength(string)
                : SETTINGS.weekdaysFull[this.DAY];
            },
            m: function (string) {
            ...
            mmm: function (string, dateObject) {
              ...
              return string
                ? getMonthLength(string, dateObject, collection)
            ...
            toArray: function (format) {
              return format.split(/(?=\b)(d{1,4}|m{1,4}|y{4}|yy)+(\b)/g);
            },
          }; //endreturn
        })(), //DATE_FORMATS
        ...
        DISABLED_DATES = (function () {
          ...
          var isDisabledDate = function (date) {
            return (
              this.TIME == date.TIME || DATES_TO_DISABLE.indexOf(this.DAY) > -1
            );
          };
          ...
          if (CALENDAR.off) {
            ...
            DATES_TO_DISABLE.map(function (loopDate) {
              ...
              if (
                loopDate.TIME < PSEUDO_DATE_MIN.TIME &&
                loopDate.TIME > DATE_MIN.TIME
              ) {
              ...
              if (
                loopDate.TIME > PSEUDO_DATE_MAX.TIME &&
                loopDate.TIME <= DATE_MAX.TIME
              ) {
        ...
        DATE_HIGHLIGHTED = (function (dateDataValue, dateEntered) {
          ...
          if (dateDataValue) {
            ...
            DATE_FORMATS.toArray(SETTINGS.formatSubmit).map(function (
              formatItem
            ) {
              ...
              var formattingLength = DATE_FORMATS[formatItem]
                ? DATE_FORMATS[formatItem](dateDataValue, dateEntered)
                : formatItem.length;
              ...
              if (DATE_FORMATS[formatItem]) {
                dateEntered[formatItem] = dateDataValue.slice(
                  0,
                  formattingLength
                );
              }
            ...
            dateEntered = [
              +(dateEntered.yyyy || dateEntered.yy),
              +(dateEntered.mm || dateEntered.m) - 1,
              +(dateEntered.dd || dateEntered.d),
            ];
          }
          ...
          return createValidatedDate(
            dateEntered && (!isNaN(dateEntered) || Array.isArray(dateEntered))
              ? dateEntered
              : DATE_TODAY
          );
        ...
        ELEMENT_HIDDEN = SETTINGS.formatSubmit
          ? $(
              ...
              ELEMENT.value ? getDateFormatted(SETTINGS.formatSubmit) : ""
            )[0]
          : null,
        ...
        $HOLDER = $(
          createNode(STRING_DIV, createCalendarWrapped(), CLASSES.holder)
        )
      ...
      function createValidatedDate(datePassed, direction, skipMonthCheck) {
        ...
        if (CALENDAR.off && !CALENDAR.offDays) {
          ...
          datePassed =
            datePassed.TIME < PSEUDO_DATE_MIN.TIME
              ? PSEUDO_DATE_MIN
              : datePassed.TIME > PSEUDO_DATE_MAX.TIME
              ? PSEUDO_DATE_MAX
              : datePassed;
        }
        ...
        // Otherwise if there are disabled dates
          ...
          while (DATES_TO_DISABLE.filter(DISABLED_DATES, datePassed).length) {
            ...
            datePassed = createDate([
              datePassed.YEAR,
              datePassed.MONTH,
              datePassed.DATE + (direction || 1),
            ]);
            ...
            if (!skipMonthCheck && datePassed.MONTH != originalDate.MONTH) {
              originalDate = datePassed = createDate([
                originalDate.YEAR,
                originalDate.MONTH,
                direction < 0 ? --originalDate.DATE : ++originalDate.DATE,
              ]);
            }
          }
        }
      ...
      function createMonthNav(next) {
        ...
        if (
          (next &&
            MONTH_FOCUSED.YEAR >= DATE_MAX.YEAR &&
            MONTH_FOCUSED.MONTH >= DATE_MAX.MONTH) ||
          (!next &&
            MONTH_FOCUSED.YEAR <= DATE_MIN.YEAR &&
            MONTH_FOCUSED.MONTH <= DATE_MIN.MONTH)
        ) {
          return "";
        }
      ...
      function createMonthLabel(monthsCollection) {
        ...
        return SETTINGS.monthSelector
            ...
            createNode(
              ...
              monthsCollection.map(function (month, monthIndex) {
                ...
                return createNode(
                  ...
                  "value=" +
                    monthIndex +
                    (MONTH_FOCUSED.MONTH == monthIndex ? " selected" : "") +
          ...
          : createNode(
              STRING_DIV,
              monthsCollection[MONTH_FOCUSED.MONTH],
              CLASSES.month
            );
      } //createMonthLabel
      ...
      function createYearLabel() {
        var yearFocused = MONTH_FOCUSED.YEAR,
        ...
        if (yearsInSelector) {
          ...
          yearsInSelector =
            yearsInSelector === true ? 5 : ~~(yearsInSelector / 2);

          var // Create a collection to hold the years
            yearsCollection = [],
            ...
            highestYear =
              yearFocused + yearsInSelector + (firstYear - lowestYear),
          ...
          firstYear = getNumberInRange(
            lowestYear - (highestYear - lastYear),
            DATE_MIN.YEAR
          );
      ...
      function createTableBody() {
        var // The loop date object
          ...
          countMonthDays = createDate([
            MONTH_FOCUSED.YEAR,
            MONTH_FOCUSED.MONTH + 1,
            0,
          ]).DATE,
          ...
          countShiftby =
            createDate([MONTH_FOCUSED.YEAR, MONTH_FOCUSED.MONTH, 1]).DAY +
            (SETTINGS.firstDay ? -2 : -1);
        ...
        for (var index = 0; index < DAYS_IN_CALENDAR; index += 1) {
          ...
          classAndBinding = createDateClassAndBinding(
            loopDate,
            pseudoIndex > 0 && pseudoIndex <= countMonthDays
          );
          ...
          if ((index % DAYS_IN_WEEK) + 1 == DAYS_IN_WEEK) {
            ...
            calendarWeeks += createNode(
              "tr",
              calendarDates.splice(0, DAYS_IN_WEEK)
            );
          }
        } //endfor
      ...
      function createDateClassAndBinding(loopDate, isMonthFocused) {
        ...
        if (
          ...
          loopDate.TIME > DATE_MAX.TIME ||
          (DATES_TO_DISABLE &&
            DATES_TO_DISABLE.filter(DISABLED_DATES, loopDate).length)
        ) {
          isDateDisabled = 1;
      ...
      function createTodayAndClear() {
        return (
          createNode(
            ...
            CLASSES.buttonToday,
            "data-date=" +
              getDateFormatted("yyyy/mm/dd", DATE_TODAY) +
              " " +
              getTabindexState()
          ) +
          createNode(
      ...
      function getNumberInRange(number, limit, upper) {
        ...
        return (upper && number < limit) || (!upper && number > limit)
          ? number
          : limit;
      } //getNumberInRange
      ...
      function getDateFormatted(format, date) {
        ...
        return DATE_FORMATS.toArray(format || SETTINGS.format)
          .map(function (value) {
            // Trigger the date formats function
            // or just return value itself.
            return (
              triggerFunction(DATE_FORMATS[value], date || DATE_SELECTED) ||
              value
            );
          })
          .join("");
